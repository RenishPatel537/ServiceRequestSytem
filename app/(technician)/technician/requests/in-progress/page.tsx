...
export default function InProgressRequestsPage() {
    ...
    useEffect(() => {
        fetch("/api/technician/in-progress")
            .then((res) => {
                if (!res.ok) throw new Error("Failed to fetch in-progress requests");
                return res.json();
            })
            .then((data) => {
                setRequests(data);
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
            <div className="flex flex-col gap-1">
                <h2 className="text-2xl font-bold text-gray-900 tracking-tight">In Progress</h2>
                <p className="text-sm text-gray-500 font-medium">Service requests you are currently working on.</p>
            </div>
            ...
            {loading ? (
                ...
                <RequestTable requests={requests} emptyMessage="No requests are currently in progress." />
            )}
        </div>
    );
}
