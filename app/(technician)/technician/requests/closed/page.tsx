...
export default function ClosedRequestsPage() {
    ...
    useEffect(() => {
        fetch("/api/technician/closed")
            .then((res) => {
                if (!res.ok) throw new Error("Failed to fetch closed requests");
                return res.json();
            })
            .then((data) => {
                setRequests(data);
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
            <div className="flex flex-col gap-1">
                <h2 className="text-2xl font-bold text-gray-900 tracking-tight">Closed</h2>
                <p className="text-sm text-gray-500 font-medium">History of closed/archived service requests.</p>
            </div>
            ...
            {loading ? (
                ...
                <RequestTable requests={requests} emptyMessage="No closed requests found." />
            )}
        </div>
    );
}
