...
import { servicerequesttype, staff, servicerequesttypewiseperson } from "@/generated/prisma/client";

interface TechnicianFormProps {
    requestTypes: servicerequesttype[];
    staffList: staff[];
    initialData?: servicerequesttypewiseperson;
    isEdit?: boolean;
}

export function TechnicianForm({ requestTypes, staffList, initialData, isEdit = false }: TechnicianFormProps) {
    const router = useRouter();
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState<string | null>(null);

    const [formData, setFormData] = useState({
        servicerequesttypeid: initialData?.servicerequesttypeid ? String(initialData.servicerequesttypeid) : "",
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const url = "/api/admin/technicians";
            const method = isEdit ? "PUT" : "POST";
            const payload = {
                ...formData,
                ...(isEdit && { servicerequesttypewisepersonid: initialData?.servicerequesttypewisepersonid }),
            };
            ...
            router.push("/admin/assignments/technicians");
            router.refresh();
        } catch (err: any) {
        ...
        <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl bg-white p-6 shadow-sm rounded-lg border border-gray-200">
            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                <div className="col-span-2 sm:col-span-1">
                    <label htmlFor="servicerequesttypeid" className="block text-sm font-medium text-gray-700">Service Request Type <span className="text-red-500">*</span></label>
                    <select
                ...
                <div className="col-span-2">
                    ...
                    <textarea
                        ...
                        onChange={handleChange}
                        placeholder="e.g. Primary Technician, Specialist"
                        className="mt-1 block w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-gray-900 shadow-sm focus:border-blue-500 focus:outline-none sm:text-sm"
                    />
                </div>
            </div>
